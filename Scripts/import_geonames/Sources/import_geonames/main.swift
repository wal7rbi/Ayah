...
// An override keyed on an id that no longer survives the country/population
// filter silently does nothing, which for a hand-curated table is how it
// rots. Warn loudly rather than failing: GeoNames legitimately retires and
// renumbers places, and that is a reason for a human to look at the table,
// not a reason to block an otherwise-good import.
let bundledIDs = Set(cities.map(\.geonameID))
let unmatchedOverrides = ArabicNameOverrides.table.keys.filter { !bundledIDs.contains($0) }.sorted()
let overrideAppliedCount = ArabicNameOverrides.table.keys.count - unmatchedOverrides.count
if !unmatchedOverrides.isEmpty {
    FileHandle.standardError.write(Data("""
    import_geonames: warning: \(unmatchedOverrides.count) of \(ArabicNameOverrides.table.count) \
    Arabic-name overrides matched no bundled city and were ignored: \
    \(unmatchedOverrides.map(String.init).joined(separator: ", "))
    Review ArabicNameOverrides.swift against the current dump.
...
let checksumPath = (options.outDir as NSString).appendingPathComponent("GEONAMES_CHECKSUM")
...
do {
    try writeCitiesDatabase(
        ...
        meta: [
            ...
            ("override_name_count", String(overrideAppliedCount)),
        ]
    )
} catch {
...
// The database is checksummed and `LocationRepository` fails closed against
// this file at every launch, so the importer has to write it — leaving it to
// be regenerated by hand means any re-import produces a database the app
// refuses to load. Same `sha256:<hex>` shape `import_quran` writes, because
// `LocationRepository.verifyFileChecksum` parses it.
guard let databaseBytes = FileManager.default.contents(atPath: dbPath) else {
    fail("could not read back the database just written at \(dbPath)")
}
let databaseChecksum = "sha256:" + SHA256.hash(data: databaseBytes)
    .map { String(format: "%02x", $0) }.joined()
do {
    try (databaseChecksum + "\n").write(toFile: checksumPath, atomically: true, encoding: .utf8)
} catch {
    fail("failed writing GEONAMES_CHECKSUM: \(error)")
}
...
- **Bundled SQLite SHA-256**: `\(databaseChecksum.dropFirst("sha256:".count))`
  (also written to `GEONAMES_CHECKSUM`, which `LocationRepository` verifies
  before runtime use)
- **Countries included**: \(countryList)
- **Arabic names**: \(arabicNameCount) of \(cities.count) cities (\(Int(100.0 * Double(arabicNameCount) / Double(cities.count)))%) have a `name_arabic`, from two sources. \(arabicNameCount - overrideAppliedCount) come from GeoNames' `alternateNamesV2` dump (`isolanguage == "ar"`, historic entries excluded, GeoNames' own `isPreferredName` flag preferred, then the shortest non-colloquial candidate — see `Scripts/import_geonames/Sources/import_geonames/ArabicNames.swift` for the exact selection logic and a documented caveat about occasional Dari/Pashto mistagging for Afghan cities). The other \(overrideAppliedCount) come from a curated override table, `ArabicNameOverrides.swift`, covering cities in Arabic-speaking countries that the dump carries no usable Arabic-tagged row for; that file states its own scope rule, what it deliberately leaves out, and the fact that it is the one part of this directory not mechanically derived from upstream data. The remainder have `name_arabic = NULL` and fall back to `name` (GeoNames' primary field, not consistently Arabic-script — e.g. transliterated Latin "Riyadh" for Saudi cities that lack a tagged Arabic alternate), which for a place with no Arabic exonym is the correct result rather than a gap.
...
print("""
Wrote \(cities.count) cities to \(dbPath)
  Arabic names: \(arabicNameCount) (\(overrideAppliedCount) from the curated override table)
  \(databaseChecksum)
""")
